...
const EmailVerificationWaiting: React.FC<EmailVerificationWaitingProps> = ({ email, onBack, isTrialUser = false }) => {
  const { user } = useAuth();
  ...
  // Check if user is verified and handle trial start or redirect
  useEffect(() => {
    if (user && user.email_confirmed_at && !trialStarted) {
      if (isTrialUser) {
        // Start trial for trial users
        handleTrialStart();
      } else {
        // Direct redirect for subscription users
        navigate('/dashboard');
      }
    }
  }, [user, navigate, isTrialUser, trialStarted]);
  ...
  const handleTrialStart = async () => {
    ...
    try {
      ...
      toast({
        title: "Welcome!",
        description: "Your free trial has started. Redirecting to dashboard...",
      });
      setTimeout(() => {
        navigate('/dashboard');
      }, 2000);
