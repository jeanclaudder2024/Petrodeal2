...
import { ShoppingCart, Plus, Edit, Trash2, Search, Sparkles, CreditCard, Shield, Users, MapPin, FileText, Image, Upload } from 'lucide-react';
...
const BuyerCompanyManagement = () => {
  ...
  const [formData, setFormData] = useState<Partial<BuyerCompany>>({});
  ...
  // Bulk upload hook
  const bulkUpload = useBulkCompanyUpload('buyer');
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      setCompanies((data || []) as BuyerCompany[]);
    } catch (error) {
      console.error('Failed to fetch buyer companies:', error);
      toast({
        title: "Error",
        description: "Failed to load buyer companies",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Success", description: editingCompany ? "Buyer company updated successfully" : "Buyer company created successfully" });
      setIsDialogOpen(false);
      resetForm();
      fetchCompanies();
    } catch (error) {
      console.error('Failed to save buyer company:', error);
      toast({
        title: "Error",
        description: "Failed to save buyer company",
        variant: "destructive"
      });
    }
  };
  ...
  const handleAutoFill = async () => {
    ...
    try {
      ...
      if (data?.success && data?.data) {
        ...
        toast({
          title: "Success",
          description: "Buyer company details auto-filled successfully!",
        });
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this buyer company?')) return;
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to delete buyer company",
        variant: "destructive"
      });
    }
  };

  const handleEdit = async (company: BuyerCompany) => {
  ...
  const handleBulkSaveCompany = async (company: GeneratedCompanyData) => {
    try {
      ...
      if (data?.id && company.bankAccounts?.length > 0) {
        ...
        await supabase.from('buyer_company_bank_accounts').insert(accountsToInsert);
      }
    ...
    <div className="space-y-6">
      <Card className="trading-card">
        <CardHeader>
          ...
          <CardDescription>
            Add, edit, and manage buyer companies (Admin only - not visible to users)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {/* Controls */}
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search buyer companies..."
                value={searchTerm}
            ...
            <Dialog open={isDialogOpen} onOpenChange={(open) => {
              ...
              <DialogContent className="max-w-5xl max-h-[95vh]">
                <DialogHeader>
                  <DialogTitle>{editingCompany ? 'Edit Buyer Company' : 'Add New Buyer Company'}</DialogTitle>
                  <DialogDescription>
                    {editingCompany ? 'Update buyer company information' : 'Enter buyer company details'}
                  </DialogDescription>
                </DialogHeader>
                <ScrollArea className="max-h-[75vh] pr-4">
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                      ...
                      <TabsContent value="basic" className="space-y-4">
                        ...
                        <p className="text-xs text-muted-foreground">
                          AI will generate realistic buyer company data including passport, bank accounts, and legal details.
                        </p>
                    ...
                    <div className="flex justify-end gap-2 pt-4 border-t">
                      ...
                      <Button type="submit">
                        {editingCompany ? 'Update Buyer Company' : 'Create Buyer Company'}
                      </Button>
                    </div>
                  </form>
                </ScrollArea>
...
export default BuyerCompanyManagement;
