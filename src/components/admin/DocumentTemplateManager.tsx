...
export default function DocumentTemplateManager() {
  ...
  const fetchVessels = async () => {
    try {
      const response = await fetch(`${getApiBaseUrl()}/vessels`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${getApiBaseUrl()}/upload-template`, {
        method: 'POST',
        body: formData,
      });
  ...
  const processDocument = async (templateId: string, vesselImo: string) => {
    try {
      ...
      const response = await fetch(`${getApiBaseUrl()}/process-document`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          setTimeout(() => {
            window.open(`${getApiBaseUrl()}/download/${result.document_id}`, '_blank');
          }, 1000);
        } else {
  ...
  const deleteTemplate = async (templateId: string) => {
    ...
    try {
      const response = await fetch(`${getApiBaseUrl()}/template/${templateId}`, {
        method: 'DELETE',
      });
